...
const AI_TOOLS = [
  {
    ...
    tag: "CONVERSATIONAL",
    description:
      "OpenAI's flagship conversational model. Handles reasoning, coding, writing, and multi-turn dialogue with exceptional context retention.",
    icon: "💬",
    color: "#10b981",
  ...
  },
  {
    ...
    tag: "ANALYTICAL",
    description:
      "Anthropic's safety-focused AI. Excels at nuanced analysis, long-document comprehension, and thoughtful, grounded reasoning.",
    icon: "🔮",
    color: "#8b5cf6",
  ...
  },
  {
    ...
    tag: "VISUAL",
    description:
      "State-of-the-art image synthesis. Transforms text prompts into photorealistic and artistic visuals with stunning aesthetic quality.",
    icon: "🎨",
    color: "#f59e0b",
  ...
  },
  {
    ...
    tag: "DEVELOPER",
    description:
      "AI pair programmer trained on billions of code lines. Autocompletes functions, generates tests, and explains unfamiliar codebases.",
    icon: "⚡",
    color: "#3b82f6",
  ...
  },
  {
    ...
    tag: "RESEARCH",
    description:
      "AI-powered search with real-time web access. Synthesizes citations into concise, sourced answers faster than traditional search.",
    icon: "🔍",
    color: "#06b6d4",
  ...
  },
  {
    ...
    tag: "CREATIVE",
    description:
      "Professional-grade AI video creation. Generate, edit, and transform video content with cinematic quality from simple prompts.",
    icon: "🎬",
    color: "#ec4899",
...
const AI_NEWS = [
  {
    title: "GPT-5 sets new reasoning benchmarks across all categories",
    source: "OpenAI Blog",
    date: "May 2026",
    dot: "#10b981",
  },
  {
    title: "Anthropic releases constitutional AI v3 with enhanced alignment",
    source: "Anthropic Research",
    date: "Apr 2026",
    dot: "#8b5cf6",
  },
  {
    title: "Google DeepMind's Gemini Ultra 2 achieves AGI-level math proofs",
    source: "DeepMind",
    date: "Mar 2026",
    dot: "#3b82f6",
  },
  {
    title: "Multimodal AI now powers 40% of enterprise software globally",
...
const GUIDE_STEPS = [
  {
    ...
    body: "The model predicts the next token using probability distributions over its vocabulary, conditioned on all prior context.",
  },
  {
    n: "02",
    title: "Pattern Learning",
    body: "Trained on massive text corpora, it learns grammar, facts, reasoning, and cultural knowledge through self-supervised objectives.",
  },
  {
    n: "03",
    title: "Alignment Tuning",
    body: "RLHF and constitutional methods shape responses to be helpful, harmless, and honest—aligned with human values.",
  },
  {
    n: "04",
    title: "Prompt Engineering",
    body: "Your input directly steers output style, depth, and format. Clear role, context, and constraints yield far better results.",
  },
];
...
function ThemeToggle({ dark, toggle }) {
  return (
    <button
      onClick={toggle}
      className={`relative h-8 w-14 rounded-full border transition-all duration-500 ${
        dark ? "border-blue-300/20 bg-slate-800" : "border-blue-300/30 bg-white"
      }`}
...
function StatCard({ label, value, suffix = "", delay }) {
  ...
  return (
    <motion.div
      ...
      className="group relative overflow-hidden rounded-2xl border border-blue-300/20 bg-white/5 p-5 backdrop-blur-sm cursor-default"
    >
...
function ToolCard({ tool, index }) {
  ...
  return (
    <motion.article
      ...
      whileHover={{ y: -7 }}
      className={`group relative overflow-hidden rounded-2xl border bg-gradient-to-br p-6 cursor-pointer transition-shadow duration-300 ${tool.accent} border-blue-300/20 ${
        hovered ? `shadow-xl ${tool.glow}` : ""
      }`}
    >
      <div
      ...
      <div className="relative">
        <div className="flex items-start justify-between">
          <div className="flex h-12 w-12 items-center justify-center rounded-xl text-xl border border-blue-300/20 bg-white/5 backdrop-blur-sm">
            {tool.icon}
          </div>
          <span
            className="rounded-full px-2.5 py-1 text-[10px] font-black tracking-widest border border-blue-300/20"
        ...
        <motion.button
          ...
          className="mt-5 flex items-center gap-2 rounded-xl px-4 py-2 text-xs font-bold tracking-wider border border-blue-300/20 transition-all duration-300"
          style={{ color: tool.color, borderColor: tool.color + "50", backgroundColor: hovered ? tool.color + "20" : "transparent" }}
        >
          EXPLORE TOOL
...
function NewsItem({ item, index }) {
  return (
    <motion.div
      ...
      transition={{ delay: 0.55 + index * 0.1, duration: 0.45, ease: EASE }}
      whileHover={{ x: 4 }}
      className="group flex items-start gap-4 rounded-xl border border-blue-300/20 bg-white/3 p-4 cursor-pointer transition-colors duration-300 hover:bg-white/6 hover:border-blue-300/30"
    >
      <div className="mt-1.5 h-2 w-2 flex-shrink-0 rounded-full" style={{ backgroundColor: item.dot }} />
      <div className="flex-1 min-w-0">
        <p className="text-sm font-semibold text-white leading-snug group-hover:text-violet-300 transition-colors duration-300">{item.title}</p>
...
function GuideStep({ step, index }) {
  return (
    <motion.div
      ...
      animate={{ opacity: 1, y: 0 }}
      transition={{ delay: 0.35 + index * 0.1, duration: 0.45, ease: EASE }}
      className="group flex gap-5 rounded-2xl border border-blue-300/20 bg-white/3 p-5 hover:bg-white/6 hover:border-blue-300/30 transition-colors duration-300"
    >
...
export default function AIPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const authRes = await fetch(`${baseUrl}/abac/authorize`, {
        method: 'POST',
      ...
      if (!authJson?.permit) {
        setDemoResult(`Access denied by ABAC. Reason: ${authJson?.reason || 'Denied'}`)
        return
      }

      const seed = demoText.length % DEMO_RESPONSES.length
      // Keep UI demo deterministic; ABAC gate happens above.
      setDemoResult(`${DEMO_RESPONSES[seed]} (Topic: "${demoText}")`)
    } catch (e) {
      setDemoResult(`Backend error: ${e?.message || 'Unknown error'}`)
    } finally {
      setGenerating(false)
    }
  };

  const bg = dark ? "bg-[#080810] text-white" : "bg-[#f4f4f8] text-slate-900";

  return (
    <div className={`min-h-screen ${bg} transition-colors duration-500`} style={{ fontFamily: "'DM Sans', 'Syne', sans-serif" }}>
      {/* ── NAV ── */}
      <motion.nav
        className={`sticky top-0 z-50 border-b backdrop-blur-xl ${dark ? "border-blue-300/20 bg-[#080810]/80" : "border-blue-300/30 bg-white/80"}`}
        transition={{ duration: 0.5, ease: EASE }}
      >
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          <div className="flex items-center gap-3">
            <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-violet-500 to-cyan-500 text-sm">✦</div>
            ...
            <div className="mt-10 flex flex-wrap items-center justify-center gap-4">
              ...
              <motion.button
                ...
                className={`flex items-center gap-2 rounded-xl border px-6 py-3 text-sm font-bold transition-colors duration-200 ${
                  dark ? "border-blue-300/30 text-white hover:border-blue-300/50 hover:bg-white/5" : "border-blue-300/40 text-slate-800 hover:border-blue-300/60 hover:bg-black/5"
                }`}
              >
                Watch Demo
        ...
        <div className="mx-auto max-w-7xl">
          <motion.div
            ...
            className={`rounded-3xl border p-8 md:p-10 relative overflow-hidden ${
              dark ? "border-blue-300/20 bg-white/3" : "border-blue-300/30 bg-black/3"
            }`}
          >
                ...
                {[
                  ...
                  <motion.div
                    ...
                    className={`rounded-xl border p-4 cursor-default ${
                      dark ? "border-blue-300/20 bg-white/4 hover:bg-white/7" : "border-blue-300/30 bg-black/3 hover:bg-black/6"
      ...
      <section id="guide" className="px-6 py-12">
        <div className="mx-auto max-w-7xl grid lg:grid-cols-2 gap-8">
          ...
          <motion.div
            ...
            viewport={{ once: true, amount: 0.3 }}
            transition={{ duration: 0.6, ease: EASE }}
            className={`rounded-3xl border p-7 ${dark ? "border-blue-300/20 bg-white/3" : "border-blue-300/30 bg-black/3"}`}
          >
            <div className="flex items-center justify-between mb-6">
              ...
              <motion.button
                ...
                className={`rounded-xl border px-3 py-2 text-xs font-bold transition-colors duration-200 ${
                  dark ? "border-blue-300/20 text-slate-400 hover:text-white hover:border-blue-300/40" : "border-blue-300/30 text-slate-500 hover:text-slate-900 hover:border-blue-300/50"
                }`}
              >
          ...
          >
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.99 }}
              transition={{ duration: 0.25, ease: EASE }}
              className={`mt-6 w-full rounded-xl border py-2.5 text-xs font-bold tracking-wider transition-colors duration-300 ${
                dark ? "border-blue-300/20 text-slate-400 hover:border-blue-300/40 hover:text-white hover:bg-white/5" : "border-blue-300/30 text-slate-500 hover:border-blue-300/50 hover:text-slate-900 hover:bg-black/5"
              }`}
            >
              VIEW ALL UPDATES →
            </motion.button>
        ...
        <div className="mx-auto max-w-3xl">
          ...
          >
            ...
            <div className="relative">
              ...
              <p className={`mt-2 text-sm ${dark ? "text-slate-400" : "text-slate-500"}`}>Type any topic and watch AI synthesize an insight in real time.</p>
              ...
              <textarea
                ...
                className={`mt-6 w-full rounded-2xl border p-4 text-sm leading-relaxed outline-none resize-none transition-colors duration-300 focus:border-blue-300/60 ${
                  dark
                    ? "border-blue-300/20 bg-white/5 text-white placeholder-slate-600 focus:bg-white/7"
                    : "border-blue-300/30 bg-black/5 text-slate-900 placeholder-slate-400 focus:bg-black/7"
                }`}
              />
              ...
              <AnimatePresence>
                {demoResult && (
                  <motion.div
                    ...
                    className={`mt-6 rounded-2xl border p-5 ${dark ? "border-blue-300/30 bg-violet-500/8" : "border-blue-300/30 bg-violet-500/5"}`}
                  >
      ...
      {/* ── FOOTER ── */}
      <footer className={`border-t px-6 py-8 ${dark ? "border-blue-300/20" : "border-blue-300/30"}`}>
        <div className="mx-auto max-w-7xl flex flex-wrap items-center justify-between gap-4">
          ...
          <p className={`text-xs ${dark ? "text-slate-600" : "text-slate-400"}`}>© 2026 AIOS · Built for the future of intelligence.</p>
        </div>
      </footer>
